...
class MyNeuralNetwork(Chain):
    def __init__(self, n_mid_units=256, n_out=64):
        ...
        with self.init_scope():
            n_mid_units = int(trial.suggest_int('n_mid_units{}'.format(i), 4, 256))
...
def create_optimizer(trial, model):
    optimizer_name = trial.suggest_categorical('optimizer', 
        ['CorrectedMomentumSGD', 'NesterovAG', 'RMSpropGraves', 'SGD'])
    if optimizer_name == 'CorrectedMomentumSGD':
        momentum_sgd_lr = trial.suggest_loguniform('momentum_sgd_lr', 1e-5, 1e-1)
        optimizer = chainer.optimizers.MomentumSGD(lr=momentum_sgd_lr)
    if optimizer_name == 'NesterovAG':
        nag_lr = trial.suggest_loguniform('nag_lr', 1e-5, 1e-1)
        optimizer = chainer.optimizers.NesterovAG(lr=nag_lr)
    if optimizer_name == 'RMSpropGraves':
        rmsg_lr = trial.suggest_loguniform('rmsg_lr', 1e-6, 1e-2)
        optimizer = chainer.optimizers.RMSpropGraves(lr=rmsg_lr)
    else:
        sgd_lr = trial.suggest_loguniform('sgd_lr', 1e-5, 1e-1)
        optimizer = chainer.optimizers.RMSpropGraves(lr=sgd_lr)
